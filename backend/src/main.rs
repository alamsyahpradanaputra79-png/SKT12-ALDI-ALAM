...
// Function to write sensor data to InfluxDB
async fn write_to_influxdb(
    ...
    let body = format!(
        "sht20_sensor,host=esp32 temperature={},humidity={} {}",
        data.temperature,
        data.humidity,
        data.timestamp
    );
    ...
    info!("✅ Sensor data written to InfluxDB successfully.");
    Ok(())
}

// Helper function to post data to InfluxDB
...
// Data structure for the last row from InfluxDB
...
// Function to get the last data point from InfluxDB
async fn get_last_data(
  ...
  |> filter(fn: (r) => r["_field"] == "temperature" or r["_field"] == "temperature_celsius" or r["_field"] == "humidity")
...
// Function to parse the CSV response from InfluxDB
...
// Function to get all sensor data from multiple buckets
async fn get_all_sensor_data(
    ...
    // Query 1: SHT20 Sensor (temperature & humidity)
...
async fn main() -> Result<()> {
    ...
    info!("========================================");
    info!("Starting Backend v2.0 (DEBUG MODE)");
    ...
    let http_client = Arc::new(
        Client::builder()
            .danger_accept_invalid_certs(true) // For local InfluxDB
            .timeout(Duration::from_secs(30))
            .build()?,
    );

    // --- Channel for Serial Data with stats ---
    ...
    tokio::spawn({
        ...
        async move {
            ...
            let on_data = move |data: SensorData| {
                ...
                info!(
                    "📡 SERIAL_TASK [#{}]: Received data from serial: T={:.1}°C, H={:.1}%",
                    count, data.temperature, data.humidity
                );
                
                match tx.try_send(data) {
                    Ok(_) => {
                        info!("✅ SERIAL_TASK [#{}]: Data sent to channel successfully", count);
                        Ok(())
                    }
                    Err(e) => {
                        error!(
                            "❌ SERIAL_TASK [#{}]: Failed to send to channel: {}",
                            count, e
                        );
    ...
    tokio::spawn({
        ...
        async move {
            info!("💾 INFLUX_WRITER_TASK: Starting and waiting for data...");
            while let Some(data) = rx.recv().await {
                let count = counter.fetch_add(1, Ordering::Relaxed) + 1;
                info!(
                    "📥 INFLUX_WRITER_TASK [#{}]: Received data from channel: T={:.1}°C, H={:.1}%",
                    count, data.temperature, data.humidity
                );
                
                info!("💾 INFLUX_WRITER_TASK [#{}]: Writing to InfluxDB...", count);
            ...
            Ok(payload) => {
                ...
                if is_connected && data_found {
                        ...
                        .await
                    {
                        ...
                        info!("✅ BRIDGE_TASK: Data published to ThingsBoard successfully!");
                    }
                } else {
                    if !is_connected {
